...
export default function BookingModal({ isOpen, room, onClose, onSuccess }) {
  ...
  // Reset form when opened for a new room
  useEffect(() => {
    if (isOpen) {
      setTitle('');
      setAttendeesCount(1);
  ...
  const fetchAvailability = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_BASE}/api/bookings/check-availability/?room_id=${room.id}&start_time=${selectedDate}T00:00:00Z&end_time=${selectedDate}T23:59:59Z`
      );
      if (res.ok) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      ...
      <div className="relative bg-white w-full max-w-4xl rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200 z-10 max-h-[92vh] flex flex-col my-auto overflow-hidden animate-in fade-in duration-200">
        {/* 1. Modal Header */}
        <div className="px-4 sm:px-6 py-3.5 sm:py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/60 sticky top-0 z-20">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 font-bold">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
          ...
          <button
            ...
            className="w-8 h-8 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-200/60 transition flex items-center justify-center shrink-0 cursor-pointer"
          >
          ...
          <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2.5 sm:gap-3 w-full sm:w-auto justify-end">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 sm:flex-initial px-4 py-2 text-xs font-semibold text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 rounded-xl transition shadow-xs text-center cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 sm:flex-initial px-5 py-2.5 text-xs font-bold text-white bg-[#0051d5] hover:bg-blue-700 active:bg-blue-800 disabled:opacity-50 rounded-xl shadow-md shadow-blue-600/25 transition flex items-center justify-center gap-1.5 cursor-pointer"
              >
                {isSubmitting ? (
                  <span>Reserving...</span>
                ) : (
